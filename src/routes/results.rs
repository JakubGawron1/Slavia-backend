...
pub async fn create_result(
...
) -> Result<Json<CompetitionResult>, (StatusCode, String)> {
    ...
    if claims.role == Role::Athlete {
        ...
        if let Some(row) = rows.next().await.map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))? {
            let athlete_id: String = row.get(0).unwrap();
            if athlete_id != payload.athlete_id {
                return Err((StatusCode::FORBIDDEN, "Athletes can only submit their own results".to_string()));
            }
        } else {
            return Err((StatusCode::NOT_FOUND, "Athlete profile not found".to_string()));
        }
    }
